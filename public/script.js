...
async function loadFlowers() {
    ...
    flowers.forEach((flower) => {
        ...
        card.innerHTML = < img src = "${flower.url}"
        alt = "${flower.nama}"
        style = "width: 100%; border-radius: 12px;" / > < p > $ {
            flower.emoji
        }
        $ {
            flower.nama
        } < /p>; card.style.cursor = 'pointer'; card.addEventListener('click', () => { selectedFlower = flower; document.querySelectorAll('.flower-card').forEach((c) => (c.style.border = 'none')); card.style.border = '3px solid var(--ios-primary)'; document.getElementById('btn-next-bunga').disabled = false; });
...
function startTasBattle() {
    tasList = tasList.sort(() => 0.5 - Math.random());
    currentIndex = 1;
...
function renderHasilAkhir() {
    ...
    const hasil = < h3 > $ {
        ...
        kamu cocok dengan $ {
            top.nama
        }🌟 < /h3> <p>Warna favoritmu: <b>${warna1}</b > & < b > $ {
            warna2
        } < /b></p > < p > Bunga pilihanmu: $ {
            bunga.emoji
        } < b > $ {
            bunga.nama
        } < /b></p > ;
